...
// Récupérer les résultats d’un examen (via l’id_examen)
...
// Récupérer les résultats par inscription
...
// Créer un résultat à partir d’une inscription et d’un sujet
exports.create = async (req, res) => {
  let { id_inscription, id_sujet, note, id_eleve, id_examen } = req.body;

  // Compatibilité : si on reçoit (id_eleve, id_examen), on cherche l’inscription correspondante
...
// Publier tous les résultats d’un examen (optionnel : ajoute d’abord la colonne "publie")
exports.publier = async (req, res) => {
  const { idExamen } = req.params;
  try {
    // Met à jour tous les résultats liés à cet examen via inscription_examens
    await db.query(
    ...
    console.error(err);
    // Si la colonne 'publie' n’existe pas, on affiche une aide
    if (err.code === 'ER_BAD_FIELD_ERROR') {
      return res.status(500).json({
        message: 'La colonne "publie" n’existe pas dans resultat_examens. Exécutez : ALTER TABLE resultat_examens ADD COLUMN publie BOOLEAN DEFAULT FALSE;'
      });
    }
    res.status(500).json({ message: 'Erreur serveur' });
  }
